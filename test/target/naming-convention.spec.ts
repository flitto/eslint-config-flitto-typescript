...
describe('Flitto Custom Naming Convention Linting Rule Test', () => {
  ...
  describe('@typescript-eslint/naming-convention', () => {
    const dir = 'test/target/naming-convention'
    it('인터페이스 이름의 Prefix 로 `I` 가 포함되어있지 않으면 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'interface_prefix_not_started_with_I.ts'))
    ...
    it('인터페이스 이름의 Prefix 로 `I` 가 포함되어있어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'interface_prefix.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('클래스의 이름이 PascalCase 가 아니라면 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'class_name_as_camel_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('클래스의 이름은 PascalCase 이어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'class_name_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('클래스의 readonly 멤버가 UPPER_CASE 가 아니라면 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'class_property_static_readonly_no_upper_casing.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('클래스의 static readonly 멤버는 UPPER_CASE 이어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'class_property_static_readonly_upper_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('클래스의 프로퍼티가 PascalCase 인 경우 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'class_property_as_pascal_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('클래스의 프로퍼티는 camelCase 이거나 snake_case 이어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'class_property_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('object 의 프로퍼티는 camelCase 이거나 snake_case 이어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'object_property_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('enum 명이 pascalCase 라면 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'enum_name_as_pascal_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('enum 프로퍼티가 UPPER_CASE 가 아니라면 린트에러가 발생합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'enum_property_not_upper_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('enum 명은 pascalCase 이거나 UPPER_CASE 이고, enum 프로퍼티은 모두 UPPER_CASE 이어야 합니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'enum_name_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })

    it('snake_case 인 변수는 허용되지 않습니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'variable_as_snake_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('PascalCase 인 변수는 허용되지 않습니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, INVALID, 'variable_as_pascal_case.ts'))
      expect(results[0].messages.length).toEqual(1)
    ...
    it('변수이름에는 camelCase, UPPER_CASE 만이 허용됩니다.', async () => {
      const results = await lint.lintFiles(Path.join(dir, VALID, 'variable_name_casing.ts'))
      expect(results[0].messages.length).toEqual(0)
    })
  })
})
